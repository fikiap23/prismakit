# Fastify + PrismaKit example

Minimal Fastify app using `@prismakit/core` and `@prismakit/memory`. **No Docker, Postgres, or Redis required** — uses an in-memory Prisma-like stub.
...
```bash
curl http://localhost:3003/users/demo-user
```
